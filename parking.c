...
/*
   ...
   of chargers M, where N < M.
    ...
    Each car can make reservations on up to M chargers. As time passes and
   chargers become free, reservations are fulfilled.

    Once a car is charging on any charger, its reservations on any other
   chargers are canceled. Once a car has finished charging, the charger is freed
   and the car is marked as charged.
...
int all_charged(int num_cars, const Car *cars) {
...
// cancel_reservation_for removes any reservation for car from charger and
// prints the cancelation to stdout
...
// make_reservation allocates memory for and returns a pointer to a new
// Reservation for car
...
// reserve adds car to charger's list of reservations and prints the reservation
// to stdout
...
// make_reservations distributes charger reservations for cars. Each car may
// reserve up to num_chargers chargers.
void make_reservations(int num_cars, Car *cars, int num_chargers,
                       Charger *chargers) {
  for (int car = 0; car < num_cars; ++car) {
    // reserve a random number of chargers, or 2 chargers, whichever is greater
    int num_reservations = imax(2, arc4random() % (num_chargers + 1));
    printf("📞 making %d reservations for %s\n", num_reservations,
           cars[car].owner);
    assert(num_reservations <= num_chargers);

    for (int charger = 0; charger < num_reservations; ++charger) {
      // TODO: randomize which chargers we pick. this loads up the first few
      // chargers and leaves the last ones idle.
      reserve(&cars[car], &chargers[charger]);
    }
  }
}

// fulfill_next_reservaion promotes charger's next reservation to charging and
// prints
...
// validate cars asserts that all cars have been initialized according to our
// assumptions
  ...
  while (!all_charged(num_cars, cars)) {
    ...
    for (int c = 0; c < NUM_CHARGERS; ++c) {
      ...
      if (charger->charging_car == 0 && charger->reservations != 0) {
        ...
        if (charger->charging_minutes ==
            charger->charging_car->charge_minutes) {
          charger->charging_car->charged = 1;
          printf("🏁 %s's car is now charged after %d minutes on charger %d.\n",
                 charger->charging_car->owner,
                 charger->charging_car->charge_minutes, charger->id);
